...
export const createDrogsSession = (key, onSignOut) => {
  ...
  const dispose = () => {
    clearTimeout(timer);
    ['pointerdown', 'pointermove', 'keydown', 'touchstart', 'scroll'].forEach(type => document.removeEventListener(type, touch, true));
    ['pageshow', 'focus'].forEach(type => window.removeEventListener(type, check));
    document.removeEventListener('visibilitychange', check);
  };
  return {active, start, signOut, dispose};
};
