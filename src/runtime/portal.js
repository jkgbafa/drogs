...
function confirmation(resignation=false){
  viewClass('record-view');
  app.innerHTML=`<div class="wrap confirmation-wrap"><section class="content-card confirmation-card"><img class="receipt-logo" src="assets/mitre-transparent.png" alt="D.R.O.G.S"><div class="success-mark" aria-hidden="true">✓</div><h1 tabindex="-1">Thank you, ${esc(current.name)}.</h1><p>Your response is being reviewed by D.R.O.G.S.</p>${resignation?'<p>Your request to resign has been recorded. The office will contact you privately.</p>':''}<div class="confirmation-actions"><button class="primary" id="confirmation-signout">Sign out</button><button class="ghost" id="profile">Return to record</button></div></section></div>`;
