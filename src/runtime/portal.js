import { assessApplication, validateApplication, visibleFields } from './application-review';
...
function declaration(){
  ...
  const readForm=form=>({...Object.fromEntries(new FormData(form).entries()),declaration:false});
  document.querySelector('#declaration-form').onchange=event=>{updateFollowUps(event.currentTarget);saveDeclaration({status:'draft',responses:readForm(event.currentTarget),updatedAt:new Date().toISOString(),review:'Awaiting submission',reviewAssessment:null})};
  document.querySelector('#declaration-form').onsubmit=event=>{event.preventDefault();const responses=readForm(event.currentTarget),errors=validateApplication(questions,responses);saveDeclaration({status:'draft',responses,updatedAt:new Date().toISOString(),review:'Awaiting submission',reviewAssessment:null});if(errors.length){document.querySelector('#form-errors').textContent='Please answer: '+errors.map(e=>e.title).join(' • ');document.querySelector('#form-errors').scrollIntoView({block:'center'});return}location.hash='review'};

}

function reviewApplication(){
 viewClass('record-view');
 const r=record(current),questions=currentType==='pastor'?PASTOR_QUESTIONS:baseQuestions,responses=r.responses||{};
 if(r.status==='submitted')return approval();
 if(validateApplication(questions,responses).length){location.hash='declaration';return}
 const answer=f=>{const v=responses[f.id],o=(f.options||[]).find(x=>typeof x==='object'&&x.value===v);return o?.label||v||'Not provided'};
 app.innerHTML=`<div class="wrap"><div class="topline"><div><div class="eyebrow">Review before submitting</div><h1>Review your application</h1><p>${esc(current.name)} · Check your answers, then confirm and submit.</p></div><button class="ghost" id="edit-application">Edit answers</button></div><section class="content-card application-preview">${questions.map((q,i)=>`<section class="review-question"><span class="qno">${i+1}</span><h3>${esc(q.title)}</h3><p>${esc(answer(q))}</p>${visibleFields([q],responses).slice(1).filter(f=>responses[f.id]).map(f=>`<div class="review-follow-up"><h4>${esc(f.label)}</h4><p>${esc(answer(f))}</p></div>`).join('')}</section>`).join('')}${['disclosure','intentionNote'].filter(id=>responses[id]).map(id=>`<section class="review-question"><h3>${id==='disclosure'?'Confidential note':'Renewal note'}</h3><p>${esc(responses[id])}</p></section>`).join('')}<form id="confirm-application"><label class="truthfulness"><input type="checkbox" id="truthfulness" required> <span>I confirm that my answers are complete and truthful to the best of my knowledge. As a minister, I commit to honesty before God and to the integrity expected of my calling.</span></label><p>Applications with no matters requiring review proceed to payment. If you request support or raise a concern, DROGS will review your application first.</p><div class="form-actions"><button class="ghost" type="button" id="save-reviewed">Save</button><span id="review-save-status" role="status"></span><button class="primary" id="confirm-submit" type="submit">Submit for approval <span>→</span></button></div></form></section></div>`;
 document.querySelector('#edit-application').onclick=()=>{location.hash='declaration'};
 document.querySelector('#save-reviewed').onclick=()=>{saveDeclaration({status:'draft',responses:{...responses,declaration:false},updatedAt:new Date().toISOString(),review:'Awaiting submission',reviewAssessment:null});document.querySelector('#review-save-status').textContent='Draft saved. Your application has not been submitted.'};
 document.querySelector('#confirm-application').onsubmit=event=>{
  event.preventDefault();if(!document.querySelector('#truthfulness').checked)return;
  const finalResponses={...responses,declaration:true},assessment=assessApplication(questions,finalResponses),now=new Date().toISOString();
  document.querySelector('#confirm-submit').disabled=true;
  try{saveDeclaration({status:'submitted',responses:finalResponses,submittedAt:now,updatedAt:now,review:assessment.review,reviewAssessment:assessment,reviewMethod:assessment.automatic?'automatic':'pending',reviewedAt:assessment.automatic?now:null,applicantMessage:assessment.automatic?'Your application is approved. You are eligible to complete your annual commitment.':null,reviewHistory:[...(r.reviewHistory||[]),{from:r.review,to:assessment.review,at:now,method:assessment.automatic?'automatic':'triage',rulesVersion:assessment.rulesVersion}]});
  submissionLoading(assessment.review==='Resignation requested'?'response':assessment.automatic?'payment':'approval');}catch{document.querySelector('#confirm-submit').disabled=false;document.querySelector('#review-save-status').textContent='Your application could not be saved. Please try again; it has not been submitted.'}
 };
}
...
function approval(){
 ...
 const message=approved?'Your application has been approved. You can now complete your annual commitment.':exception?'Your application needs an individual review. Payment remains locked until your application is approved.':'Your application is being reviewed. Please be on standby for DROGS to arrange a time to discuss the matters you raised. Payment will become available once your application is approved.';
 app.innerHTML=`<div class="wrap confirmation-wrap"><section class="content-card confirmation-card"><img class="receipt-logo" src="assets/mitre-transparent.png" alt="DROGS"><div class="eyebrow">${esc(heading)}</div><h1 tabindex="-1">Thank you, ${esc(current.name)}.</h1><h2>${esc(heading)}</h2><p>${esc(r.applicantMessage||message)}</p><div class="confirmation-actions">${approved?`<button class="primary" id="approval-payment">${r.paid?'View confirmation':'Continue to payment'}</button>`:'<button class="primary" id="check-approval">Check approval status</button>'}<button class="ghost" id="profile">Return to record</button><button class="ghost" id="confirmation-signout">Sign out</button></div></section></div>`;
